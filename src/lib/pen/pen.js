...
(function(root, doc) {
  ...
  var effectNodeReg = /(?:[pubia]|h[1-6]|blockquote|[uo]l|li|strong)/i;
  ...
  function initToolbar(ctx) {
    var icons = '', inputStr = '<input class="pen-input" placeholder="Enter URL" />';
    ...
    if (icons) {
      ...
      ctx.config.editor.parentElement.appendChild(ctx._menu);
    }
  ...
  function initEvents(ctx) {
    ...
    // prevent loosing of focus when click on toolbar
    addListener(ctx, toolbar, 'mousedown', function(e) {
  ...
  Pen.prototype.menu = function() {
    ...
    // compute toolbar position in editor's bounds
    var editorBounds = this.config.editor.getBoundingClientRect();
    var x = left - (menu.clientWidth / 2);
    if (x + menu.clientWidth > editorBounds.right) {
      x = (editorBounds.right - menu.clientWidth);
    } else {
      x = Math.max(x, editorBounds.left);
    }
    menuOffset.x = left - x;
    ...
    stylesheet.insertRule('.pen-menu:after {left: ' + menuOffset.x + 'px;}', 0);
    ...
    menu.style.top = (menuOffset.y - editorBounds.top)+ 'px';
    menu.style.left = (x - editorBounds.left)+ 'px';
  ...
  Pen.prototype.toMd = function() {
    function visitNode(elem) {
      if (elem.tagName === 'UL' || elem.tagName === 'OL') {
        return '\n'+Array.from(elem.children).map(function(e, i) {
          var listPrefix = elem.tagName === 'OL'? (i+1)+'. ' : '* ';
          if (elem.parentElement.tagName === 'LI') {
            listPrefix = '  '+listPrefix;
          }
          return listPrefix+trim(visitNode(e, true));
        }).join('\n')+'\n\n';
      } else if (elem.tagName === 'P' || elem.tagName === 'DIV' || elem.tagName === 'LI') {
        return Array.from(elem.childNodes).map(function(e) {
          return visitNode(e);
        }).join('')+'\n\n';
      } else {
    ...
    var html = visitNode(this.config.editor);
    for(var p in regs) {
